...
* **Because the rules are Boolean**\ `^`_\ **, they are interpretable and testable.** Each edge in the network represents a direct, physical regulatory interaction between a transcription factor and its target gene. Nonlinear relationships between transcription factors and their targets can be captured in the inferred rules, but only direct TF-target relationships are represented in the network.
